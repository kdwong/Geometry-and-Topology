...
\begin{proposition}[Quotient Topology] \label{prop:quotient_topology} 
Let \((X, \mathcal{T})\) be a topological space, and let \(\sim\) be an equivalence relation on \(X\). 
Define the canonical projection map
\[
p : X \to X/\!\sim, 
\qquad x \mapsto [x],
\]
which assigns each point \(x \in X\) to its equivalence class \([x]\).  
Define a family of subsets \(\widetilde{\mathcal{T}}\) of \(X/\!\sim\) by
\[
\widetilde{U} \subseteq X/\!\sim \;\; \text{is in } \widetilde{\mathcal{T}}
\quad \Longleftrightarrow \quad p^{-1}(\widetilde{U}) \in \mathcal{T}.
\]
Then \(\widetilde{\mathcal{T}}\) is a topology on \(X/\!\sim\).  

We call \((X/\!\sim, \widetilde{\mathcal{T}})\) the \emph{quotient space}, and \(\widetilde{\mathcal{T}}\) the \emph{quotient topology}.
\end{proposition}
...
\begin{enumerate}
    \item Since 
    \[
    p^{-1}(X/\!\sim) = X \in \mathcal{T}, 
    \qquad 
    p^{-1}(\varnothing) = \varnothing \in \mathcal{T},
    \]
    it follows that \(X/\!\sim, \varnothing \in \widetilde{\mathcal{T}}\).
    
    \item Let \(\widetilde{U}, \widetilde{V} \in \widetilde{\mathcal{T}}\). Then 
    \[
    p^{-1}(\widetilde{U}), \; p^{-1}(\widetilde{V}) \in \mathcal{T},
    \]
    so 
    \[
    p^{-1}(\widetilde{U} \cap \widetilde{V}) 
    = p^{-1}(\widetilde{U}) \cap p^{-1}(\widetilde{V}) \in \mathcal{T}.
    \]
    Hence \(\widetilde{U} \cap \widetilde{V} \in \widetilde{\mathcal{T}}\).
    
    \item For any collection \(\{ \widetilde{U}_i \}_{i \in I} \subseteq \widetilde{\mathcal{T}}\), we have
    \[
    p^{-1}\!\left(\bigcup_{i \in I} \widetilde{U}_i\right) 
    = \bigcup_{i \in I} p^{-1}(\widetilde{U}_i).
    \]
    Since \(\mathcal{T}\) is closed under arbitrary unions, the right-hand side is in \(\mathcal{T}\).  
    Thus \(\bigcup_{i \in I} \widetilde{U}_i \in \widetilde{\mathcal{T}}\).
\end{enumerate}
Therefore, \(\widetilde{\mathcal{T}}\) is a topology on \(X/\!\sim\).
\end{proof}

\begin{remark}
1. \autoref{prop:quotient_topology} claims that \(\widetilde{U}\) is open in \(X/ \sim\) iff \({p}^{-1}\left( \widetilde{U}\right)\) is open in \(X\). The general question is that, does \(p\left( U\right)\) is open in \(X/ \sim\), given that \(U\) is open in \(X\) ? This may not necessarily hold. (See \autoref{ex:mobius_not_open}) In general \({p}^{-1}\left( {p\left( U\right) }\right)\) is strictly larger than \(U\), and may not be necessarily open in \(X\), even when \(U\) is open.
...
\begin{definition}[Open mapping]\label{def:open_mapping}
A function \(f:X\to Y\) between topological spaces is \emph{open} if for every open set \(U\subseteq X\), the image \(f(U)\) is open in \(Y\).
\end{definition}

\begin{example}\label{ex:mobius_not_open}
Let \(p:[0,1]\times[0,1]\to M:=([0,1]\times[0,1])/\!\sim\) be the quotient map to the Möbius band, where \((0,y)\sim(1,1-y)\).
Then \(p\) is \emph{not} an open mapping.

Indeed, let \(U:=B_{1/2}((0,0))\cap([0,1]\times[0,1])\), which is open in the square (subspace topology).
By the quotient definition, \(p(U)\) is open in \(M\) iff \(p^{-1}(p(U))\) is open in \([0,1]\times[0,1]\).
But
\[
p^{-1}\big(p(U)\big)
=\text{sat}(U)
=U\ \cup\ \bigl(\{1\}\times(1/2,1]\bigr),
\]
because for each \((0,y)\in U\) (which forces \(0\le y<1/2\)) its equivalent point on the right edge is \((1,1-y)\in\{1\}\times(1/2,1]\).
The set \(\{1\}\times(1/2,1]\) is not open in the subspace topology of the square (points on the boundary \(x=1\) have no two-sided neighborhood contained in that vertical segment), hence \(p^{-1}(p(U))\) is not open. Therefore \(p(U)\) is not open in \(M\), and \(p\) is not an open map.
\end{example}
